...
namespace Drupal\tokenized_auth\Controller;
  ...
  public function login(Request $request) {
    ...
    if ($uid = $this->userAuth->authenticate($credentials['name'], $credentials['pass'])) {
      ...
      if ($user->get('uid')->access('view', $user)) {
        $response_data['current_user'] = $user->toArray();
      }
      $response_data['csrf_token'] = \Drupal::service('tokenized_auth.token_storage')->generateToken($uid);
