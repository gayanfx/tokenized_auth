 ...
 *
 * @RestResource(
 *   id = "tokenized_auth_user_registration",
 *   label = @Translation("Tokenized auth user registration"),
 *   serialization_class = "Drupal\user\Entity\User",
 *   uri_paths = {
 *     "https://www.drupal.org/link-relations/create" = "/tokenized_auth/user/register",
 *   },
 * )
 */
class UserRegistrationResource extends ResourceBase {
  ...
  public function post(UserInterface $account = NULL) {
    ...
    $response['current_user'] = $account;
    if ($this->userSettings->get('register') == USER_REGISTER_VISITORS && !$this->userSettings->get('verify_mail')) {
      $response['csrf_token'] = \Drupal::service('tokenized_auth.token_storage')->generateToken($account->id());
    }
